...
* Create `/home/stm/simple-task-manager/` with following sub-folders and files:
    ...
    * All other files needed (e.g. systemd services, the `docker-compose.yml` etc.)
...
* Get the required compose file using **one** of the following ways:
    ...
    * Adjust the file to your needs. The file from the repository is not meant to be directly used in production.
...
The following `docker-compose.yml` is a rather minimal example and similar to the one used by the hosted version of STM.
It assumes an `.env` file with the necessary variables.
Additional settings (e.g. `restart` or `logging`) and environment variables might be needed depending on your setup.
...
services:
  ...
  stm-test-server:
    image: simpletaskmanager/stm-server:...
    container_name: stm-test-server
    environment:
      - STM_DB_USERNAME=${STM_TEST_DB_USERNAME}
      - STM_DB_HOST=${STM_TEST_DB_HOST}
      - STM_DB_DATABASE=${STM_TEST_DB_DATABASE}
    volumes:
      - ./server-config.json:/stm-server/config.json

  stm-test-client:
    image: simpletaskmanager/stm-client:...
    container_name: stm-test-client
    restart: unless-stopped

  stm-test-db:
    image: postgres:...
    container_name: stm-test-db
    environment:
      - POSTGRES_USER=${STM_TEST_DB_USERNAME}
    volumes:
...
* `*.json`: A json file for the server configuration. Configured in the `.env` file with `STM_SERVER_CONFIG=./path/to/server.json`.
  * This file must be specified, otherwise the server won't work.
* `*.conf`: A conf file for the nginx server. Configured in the `.env` file with `STM_NGINX_CONFIG=./path/to/nginx.conf`.
  * This is optional, the default nginx config is already working.
...
### Preparations when using a reverse proxy
...
The JSON config mus therefore contain an entry like this:
...
Specifying this file is optional, the default within the pre-built docker container is already working.
...
The `STM_...` entry stands for all kinds of configurations for the server (s. table above).
...
server {
	...
	ssl_certificate /etc/letsencrypt/live/stm-test.hauke-stieler.de/fullchain.pem;
