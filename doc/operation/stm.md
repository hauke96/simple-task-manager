...
| `is-behind-proxy`          | `STM_IS_BEHIND_PROXY`          | 'false'                                            | No        |                        | When `true`, HTTPS will never be used, even with configured HTTPS-URL and SSL-keys. This enables the server to run behind a reverse proxy.       |
