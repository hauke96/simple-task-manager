...
PROJECT_DATA=$(psql -h $STM_DB_HOST -U $STM_DB_USERNAME -t -A -c "SELECT id,users,owner FROM projects;" stm \
	| tr -d '}' \
	| tr -d '{')
...
TASK_DATA=$(psql -h $STM_DB_HOST -U $STM_DB_USERNAME -t -A -c "SELECT id,assigned_user FROM tasks;" stm \
	| grep -v "|$")
