...
export class UserInvitationComponent implements OnInit {
  ...
  public onInvitationButtonClicked() {
    if (this.users.map(u => u.name).includes(this.enteredUserName)) {
      this.notificationService.addWarning('User ${ this.enteredUserName } ' + $localize`:@@WARN_ALREADY_MEMBER:is already a member of this project`);
      return;
    }
    ...
    this.userService.getUserByName(this.enteredUserName).subscribe(
      user => {
        ...
        console.error(err);
        this.notificationService.addError($localize`:@@ERROR_USER_ID:Could not load user ID for user '` + this.enteredUserName + '\'');
      });
  }
}
